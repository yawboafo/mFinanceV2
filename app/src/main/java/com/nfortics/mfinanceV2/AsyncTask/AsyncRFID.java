...
import com.nfortics.mfinanceV2.logic.RFIDAPI;
...
public class AsyncRFID extends Handler {
	...
	public AsyncRFID(Looper looper, CommunicateManager communicateManager) {
		mWorkerThreadHandler = createHandler(looper);
		this.communicateManager = communicateManager;
		rfidAPI = new RFIDAPI(communicateManager);
	}
	...
	protected class WorkerHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case INIT:
				...
				AsyncRFID.this.obtainMessage(INIT, resetCard, -1)
						.sendToTarget();
				break;
			case GET_RANDOM_NUM:
	...
	private void switchStatus() {
		byte[] commands = new byte[7 + SWITCH_COMMAND.length];
		commands[0] = 2;
		commands[1] = 0;
		commands[2] = 0;
		commands[3] = 0;
		commands[4] = 1;
		commands[5] = 0;
		commands[6] = (byte) SWITCH_COMMAND.length;
		for (int i = 0; i < SWITCH_COMMAND.length; i++) {
			commands[7 + i] = SWITCH_COMMAND[i];
		}
