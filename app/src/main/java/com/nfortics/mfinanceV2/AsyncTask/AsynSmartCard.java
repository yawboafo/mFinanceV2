...
import com.nfortics.mfinanceV2.logic.SmartCardAPI;


public class AsynSmartCard extends Handler {

	public static final int SCARD_LOAN = 0x0001; /* !< ����Ӧ�� */
	public static final int SCARD_DEBIT = 0x0002; /* !< ���Ӧ�� */
	public static final int SCARD_QUASI_CREDIT = 0x0003; /* !< ׼��Ӧ�� */
	public static final int SCARD_ELECTRONIC_CASH = 0x0004; /* !< �����ֽ�Ӧ�� */
	...
	public AsynSmartCard(Looper looper, CommunicateManager communicateManager) {
		mWorkerThreadHandler = createHandler(looper);
		this.communicateManager = communicateManager;
		SmartCard = new SmartCardAPI(communicateManager);
	}
	...
	public void External_Authentication() {
		...
		byte[] keycode = { 0x57, 0x41, 0x54, 0x43, 0x48, 0x44, 0x41, 0x54,
	...
	public void ReadIBANCode(int PBOC_AID) {
		...
		if (ret == SmartCard.SCARD_COMMAND_EXECUTED_CORRECTLY) {
			...
			if (ret == SmartCard.SCARD_COMMAND_EXECUTED_CORRECTLY) {
				...
				System.out.println("0000000000000000000 ret=" + ret);
				onReadIBANListener.OnReadIBANFail(ret);
			}
		} else {
			System.out.println("1111111111111111111111 ret=" + ret);
			onReadIBANListener.OnReadIBANFail(ret);
		}
	}
	...
	public void setOnExternalAuthListener(
			OnExternalAuthListener onExternalAuthListener) {
	...
	public void setOnCreateCardListener(
			OnCreateCardListener onCreateCardListener) {
		this.onCreateCardListener = onCreateCardListener;
	}

	public void setOnDeleteCardListener(
	...
	public void initcard() {
		...
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_INITCARD)
				.sendToTarget();
	}

	public void externalauth() {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_EXTERNALAUTH)
				.sendToTarget();
	}

	public void readbinary(int count) {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_READBINARY, count, 0,
				null).sendToTarget();
	}

	public void readrecord(int count) {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_READRECORD, count, 0,
				null).sendToTarget();
	}

	public void writebinary(byte[] data, int count) {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_WRITEBINARY, count, 0,
				data).sendToTarget();
	}

	public void writerecord(byte[] data, int count) {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_WRITERECORD, count, 0,
				data).sendToTarget();
	}

	public void createcard() {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_CREATECARD)
				.sendToTarget();
	}

	public void deletecard() {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_DELETECARD)
				.sendToTarget();
	}

	public void ReadIBAN(int PBOC_AID) {
		mWorkerThreadHandler.obtainMessage(ASYNSMARTCARD_READIBAN, PBOC_AID, 0,
				null).sendToTarget();
	}
